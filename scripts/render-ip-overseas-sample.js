...
async function main() {
  const common = {
    ...
    REVENUE_MODEL: "Royalty with minimum guarantee",
    ...
    INITIAL_RELEASE_DATE: "September 1, 2026",
    CURRENCY: "EUR",
  ...
  const licenseOut = await renderTemplate({
    ...
    variables: {
      ...
      LICENSE_SCOPE: [
        "Translation, localization, manufacture, distribution, and marketing rights",
        "Approved for German-language tabletop products and related promotional materials",
        "Digital companion app limited to rules reference and score support",
      ].join("\n"),
      IP_PRODUCT_SCOPE: "Localized boxed board game, expansion pack, and organized-play promo kit",
      ROYALTY_TERMS: [
        "Royalty: 12% of Net Sales",
        "Minimum Guarantee: EUR 35,000 recoupable",
        "Reporting: quarterly within 30 days from each quarter end",
        "Payment: within 15 days after report submission",
      ].join("\n"),
      SUBLICENSE_ALLOWED: "Permitted only with Licensor's prior written consent.",
      TITLE_TRANSFER_MODEL: "All original IP remains with Licensor. Localized files and translations are owned by Licensee, with Licensor receiving a perpetual use right for the licensed territory version.",
      INVENTORY_SELLOFF: "180-day sell-off period after expiration or termination, subject to payment of accrued royalties.",
      SPECIAL_NOTES: "Sample output with Schedule 1 active and Schedule 2 crossed out.",
    },
  });
  ...
  const productOut = await renderTemplate({
    ...
    variables: {
      ...
      LICENSE_SCOPE: "Rights are limited to sale and distribution of finished goods supplied by Licensor.",
      IP_PRODUCT_SCOPE: [
        "Finished goods supplied by Licensor from Japan",
        "German-language packaging sticker and importer label only",
        "No local manufacture without separate written amendment",
      ].join("\n"),
      ROYALTY_TERMS: [
        "Supply price list applies instead of running royalty",
        "30% advance upon purchase order, 70% before shipment",
        "Annual MPR target: 8,000 units",
      ].join("\n"),
      SUBLICENSE_ALLOWED: "No sublicensing. Appointment of local resellers is permitted in the ordinary course of distribution.",
      TITLE_TRANSFER_MODEL: "Licensor retains all IP and manufacturing assets. Title to finished goods transfers under agreed delivery terms.",
      INVENTORY_SELLOFF: "Sell-off allowed for in-stock finished goods for 120 days after termination.",
      SPECIAL_NOTES: "Sample output with Schedule 2 active and Schedule 1 crossed out.",
    },
  });
